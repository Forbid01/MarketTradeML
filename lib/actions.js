...
export async function getOrderMessages(orderId, cursor) {
  const u = await uid(); if (!u) return { error: "Нэвтэрнэ үү" };
  try {
    const params = [orderId, u];
    let cursorClause = "";
    const afterSeq = cursor?.afterSeq;
    if (afterSeq != null && /^\d+$/.test(String(afterSeq))) {
      params.push(String(afterSeq));
...
// Худалдан авагч өөрөө QPay-аас төлбөрөө шалгуулах (callback хоцорсон/ирээгүй үед шууд баталгаажуулна).
// confirm_payment idempotent (FOR UPDATE + status='created' шалгалт) тул дахин дуудахад аюулгүй;
...
export async function checkPaymentNow(orderId, kind) {
  ...
  try {
    ...
    const check = await qpay.checkPayment(order.qpay_invoice_id);
    const { payments, paidTotal } = qpay.summarizePaidRows(check);
    if (!payments.length) return { ok: true, result: "no_payment" };
    ...
    if (isBoost) {
      ...
      result = rows[0].r;
      revalidatePath(`/boost/${orderId}`);
    } else {
      const rows = await query(`select public.confirm_payment($1,$2,$3,$4::jsonb,$5::jsonb) as r`,
...
export async function createBoostOrder(service, config) {
  ...
  try {
    ...
    if (code) {
      // Атомик: хүчинтэй бол used_count-ийг ИЖИЛ statement-д нэмж percent_off буцаана.
      // Шалгалт ба нэмэгдүүлэлт салангид биш тул TOCTOU race + max_uses хэтрэлтээс сэргийлнэ.
      const p = await queryOne(
        `update public.promo_codes set used_count = used_count + 1
          where code = $1 and active = true
            and (expires_at is null or expires_at > now())
            and (max_uses is null or used_count < max_uses)
         returning percent_off`, [code]
      );
      if (!p) return { error: "Промо код хүчингүй байна" };
...
export async function getBoostMessages(boostId, cursor) {
  const u = await uid(); if (!u) return { error: "Нэвтэрнэ үү" };
  try {
    const params = [boostId, u];
    let cursorClause = "";
    const afterSeq = cursor?.afterSeq;
    if (afterSeq != null && /^\d+$/.test(String(afterSeq))) {
      params.push(String(afterSeq));
    ...
    const rows = await query(
      `select m.id, m.sender_id, m.body, m.created_at, m.seq from public.messages m
        where m.boost_order_id = $1
          and ( exists (select 1 from public.boost_orders bo where bo.id = $1 and $2 in (bo.buyer_id, bo.booster_id))
                or exists (select 1 from public.users uu where uu.id = $2 and uu.role = 'admin') )
          ${cursorClause}
        order by m.seq asc`,
      params
