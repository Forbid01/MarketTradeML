...
// ───────────── Listings ─────────────
export async function listActiveListings({ q, rank, server, sort, minPrice, maxPrice, verified, winRate, level } = {}) {
...
export async function getRecentSales(limit = 8) {
  const rows = await query(
    `select id, title, price from public.listings
      where status = 'sold' order by updated_at desc limit $1`,
    [limit]
  );
