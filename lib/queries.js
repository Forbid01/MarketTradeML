// Серверийн талын УНШИХ функцууд (server component-уудад). RLS байхгүй тул эрхийн
// шалгалтыг WHERE/параметрээр энд эсвэл дуудагч хуудсанд хийнэ.
import { cache } from "react";
import { unstable_cache } from "next/cache";
import { query, queryOne } from "@/lib/db";

const money = (v) => (v == null ? v : Number(v));

// ───────────── Listings ─────────────
export const BROWSE_PAGE_SIZE = 48;

// Хуудаслалттай (offset) идэвхтэй зарын жагсаалт. total = шүүлтэд тохирох НИЙТ тоо
// (count(*) over()), order-т l.id tie-break — ижил үнэтэй мөрүүд хуудас дамнан
// давхардах/алгасахаас сэргийлнэ. { items, total, page } буцаана.
export async function listActiveListings({ q, rank, server, sort, minPrice, maxPrice, verified, winRate, level, page } = {}) {
  const where = ["l.status = 'active'", "l.deleted_at is null"];
  const params = [];
  if (rank)   { params.push(rank);   where.push(`l.rank = $${params.length}`); }
  if (server) { params.push(server); where.push(`l.server = $${params.length}`); }
  if (q)      { params.push(`%${q}%`); where.push(`l.title ilike $${params.length}`); }
  const mn = parseInt(minPrice, 10);
  const mx = parseInt(maxPrice, 10);
  if (Number.isFinite(mn) && mn > 0) { params.push(mn); where.push(`l.price >= $${params.length}`); }
  if (Number.isFinite(mx) && mx > 0) { params.push(mx); where.push(`l.price <= $${params.length}`); }
  const wr = parseFloat(winRate);
  if (Number.isFinite(wr) && wr > 0) { params.push(wr); where.push(`l.win_rate >= $${params.length}`); }
  const lv = parseInt(level, 10);
  if (Number.isFinite(lv) && lv > 0) { params.push(lv); where.push(`l.level >= $${params.length}`); }
  if (verified) where.push(`p.is_verified = true`);
  const order = sort === "price_asc" ? "l.price asc"
              : sort === "price_desc" ? "l.price desc"
              : sort === "rating" ? "p.rating_avg desc nulls last, l.created_at desc"
              : "l.created_at desc";
  const pg = Math.max(1, parseInt(page, 10) || 1);
  params.push(BROWSE_PAGE_SIZE);
  const limitIdx = params.length;
  params.push((pg - 1) * BROWSE_PAGE_SIZE);
  const offsetIdx = params.length;
  const rows = await query(
    `select l.id, l.title, l.price, l.server, l.rank, l.seller_id, l.created_at,
            (select url from public.listing_images li where li.listing_id = l.id order by li.sort_order limit 1) as image_url,
            p.display_name, p.is_verified, p.rating_avg, p.trades_count,
            count(*) over() as total_count
       from public.listings l
       left join public.public_profiles p on p.id = l.seller_id
      where ${where.join(" and ")}
      order by ${order}, l.id desc limit $${limitIdx} offset $${offsetIdx}`,
    params
  );
  return {
    items: rows.map((r) => ({
      id: r.id, title: r.title, price: money(r.price), server: r.server, rank: r.rank, seller_id: r.seller_id,
      createdAt: r.created_at,
      imageUrl: r.image_url ?? null,
      seller: { display_name: r.display_name, is_verified: r.is_verified, rating_avg: r.rating_avg, trades_count: r.trades_count },
    })),
    total: rows.length ? Number(rows[0].total_count) : 0,
    page: pg,
  };
}

// React cache(): нэг хүсэлтэд generateMetadata + page хоёулаа дууддаг тул
// давхар DB round-trip-ээс сэргийлнэ.
export const getListingById = cache(async function getListingById(id) {
  const listing = await queryOne(
    `select * from public.listings where id = $1 and deleted_at is null`, [id]
  );
  if (!listing) return null;
  listing.price = money(listing.price);
  const images = await query(
    `select id, url, sort_order from public.listing_images where listing_id = $1 order by sort_order`, [id]
  );
  return { ...listing, images };
});

export async function getSellerActiveListings(sellerId) {
  const rows = await query(
    `select l.id, l.title, l.price, l.server, l.rank, l.seller_id, l.created_at,
            (select url from public.listing_images li where li.listing_id = l.id order by li.sort_order limit 1) as image_url
       from public.listings l
      where l.seller_id = $1 and l.status = 'active' and l.deleted_at is null
      order by l.created_at desc`, [sellerId]
  );
  return rows.map((r) => ({
    id: r.id, title: r.title, price: money(r.price), server: r.server, rank: r.rank, seller_id: r.seller_id,
    createdAt: r.created_at,
    imageUrl: r.image_url ?? null,
  }));
}

// ───────────── Profiles / reviews ─────────────
// cache(): generateMetadata + хуудас хоёулаа дууддаг — нэг хүсэлтэд нэг л query.
export const getPublicProfile = cache(async function getPublicProfile(id) {
  return queryOne(
    `select id, display_name, avatar_url, is_verified, rating_avg, trades_count, created_at
       from public.public_profiles where id = $1`, [id]
  );
});

export async function getProfileVerified(id) {
  return queryOne(`select id, is_verified from public.public_profiles where id = $1`, [id]);
}

export async function getSellerReviews(sellerId, limit = 10) {
  return query(
    `select id, stars, comment, created_at from public.reviews
      where seller_id = $1 order by created_at desc limit $2`, [sellerId, limit]
  );
}

// ───────────── Favorites ─────────────
export async function isFavorited(userId, listingId) {
  if (!userId) return false;
  const row = await queryOne(
    `select 1 from public.favorites where user_id = $1 and listing_id = $2`, [userId, listingId]
  );
  return Boolean(row);
}

export async function listFavorites(userId) {
  const rows = await query(
    `select l.id, l.title, l.price, l.server, l.rank, l.seller_id, l.created_at, l.status,
            (select url from public.listing_images li where li.listing_id = l.id order by li.sort_order limit 1) as image_url,
            p.display_name, p.is_verified, p.rating_avg, p.trades_count
       from public.favorites f
       join public.listings l on l.id = f.listing_id and l.deleted_at is null
       left join public.public_profiles p on p.id = l.seller_id
      where f.user_id = $1
      order by f.created_at desc limit 96`, [userId]
  );
  return rows.map((r) => ({
    id: r.id, title: r.title, price: money(r.price), server: r.server, rank: r.rank, seller_id: r.seller_id,
    createdAt: r.created_at,
    status: r.status,
    imageUrl: r.image_url ?? null,
    seller: { display_name: r.display_name, is_verified: r.is_verified, rating_avg: r.rating_avg, trades_count: r.trades_count },
  }));
}

// ───────────── Orders ─────────────
export async function listOrders(userId) {
  const rows = await query(
    `select o.id, o.status, o.amount, o.fee, o.inspection_ends, o.created_at, o.buyer_id, o.seller_id, l.title as listing_title
       from public.orders o join public.listings l on l.id = o.listing_id
      where o.buyer_id = $1 or o.seller_id = $1
      order by o.created_at desc limit 200`, [userId]
  );
  return rows.map((r) => ({ ...r, amount: money(r.amount), fee: money(r.fee) }));
}

// Эрхтэй (buyer/seller/admin) бол захиалга буцаана, эс бөгөөс null.
export async function getOrderForParty(orderId, userId, admin = false) {
  const o = await queryOne(
    `select o.*, l.title as listing_title
       from public.orders o join public.listings l on l.id = o.listing_id
      where o.id = $1`, [orderId]
  );
  if (!o) return null;
  if (!admin && o.buyer_id !== userId && o.seller_id !== userId) return null;
  o.amount = money(o.amount); o.fee = money(o.fee);
  return o;
}

// Эдгээр нь захиалгын дэлгэрэнгүй унших функцууд. Эзэмшлийн шалгалтыг ДОТОР нь хийнэ
// (зөвхөн дуудагч хуудсанд найдахгүй) — userId захиалгын тал, эсвэл admin=true байх ёстой.
function partyGate() {
  return `($3 or exists (select 1 from public.orders o where o.id = $1 and $2 in (o.buyer_id, o.seller_id)))`;
}

export async function getChecklist(orderId, userId, admin = false) {
  return queryOne(
    `select tc.* from public.transfer_checklist tc where tc.order_id = $1 and ${partyGate()}`,
    [orderId, userId, admin]
  );
}

export async function getLatestDispute(orderId, userId, admin = false) {
  return queryOne(
    `select d.* from public.disputes d where d.order_id = $1 and ${partyGate()}
      order by d.created_at desc limit 1`,
    [orderId, userId, admin]
  );
}

export async function getMessages(orderId, userId, admin = false) {
  return query(
    `select m.id, m.sender_id, m.body, m.created_at, m.seq from public.messages m
      where m.order_id = $1 and ${partyGate()} order by m.seq asc`,
    [orderId, userId, admin]
  );
}

export async function getReviewForOrder(orderId, userId, admin = false) {
  return queryOne(
    `select r.id, r.stars, r.comment from public.reviews r where r.order_id = $1 and ${partyGate()}`,
    [orderId, userId, admin]
  );
}

// ───────────── Notifications ─────────────
export async function listNotifications(userId, limit = 50) {
  return query(
    `select id, type, title, body, order_id, boost_order_id, is_read, created_at
       from public.notifications where user_id = $1 order by created_at desc limit $2`,
    [userId, limit]
  );
}

export async function getUnreadCount(userId) {
  const row = await queryOne(
    `select count(*)::int as n from public.notifications where user_id = $1 and is_read = false`, [userId]
  );
  return row?.n ?? 0;
}

// ───────────── Admin worklist ─────────────
export async function listOpenDisputes() {
  return query(
    `select d.id, d.reason, d.status, d.created_at, d.order_id, l.title as listing_title
       from public.disputes d
       join public.orders o on o.id = d.order_id
       join public.listings l on l.id = o.listing_id
      where d.status = 'open' order by d.created_at asc`
  );
}

export async function listPendingPayouts() {
  const rows = await query(
    `select o.id, o.amount, o.fee, o.created_at, l.title as listing_title
       from public.orders o join public.listings l on l.id = o.listing_id
      where o.status = 'completed' and o.payout_status = 'pending'
      order by o.created_at asc`
  );
  return rows.map((r) => ({ ...r, amount: money(r.amount), fee: money(r.fee) }));
}

// ───────────── Boost orders ─────────────
export async function getBoostOrder(id, userId, admin = false) {
  const o = await queryOne(
    `select bo.*, u.display_name as booster_name,
            (select round(avg(stars)::numeric, 2) from public.boost_reviews br where br.booster_id = bo.booster_id) as booster_rating,
            (select count(*)::int from public.boost_reviews br where br.booster_id = bo.booster_id) as booster_reviews
       from public.boost_orders bo
       left join public.users u on u.id = bo.booster_id
      where bo.id = $1`, [id]
  );
  if (!o) return null;
  if (!admin && o.buyer_id !== userId && o.booster_id !== userId) return null;
  o.amount = money(o.amount);
  return o;
}

export async function getBoostReview(boostId) {
  return queryOne(`select id, stars, comment from public.boost_reviews where boost_order_id = $1`, [boostId]);
}

// Boost чатын анхны мессежүүд (server render). Эзэмшлийн гейт: buyer/booster ЭСВЭЛ admin.
export async function getBoostChat(boostId, userId, admin = false) {
  return query(
    `select m.id, m.sender_id, m.body, m.created_at, m.seq from public.messages m
      where m.boost_order_id = $1
        and ($3 or exists (select 1 from public.boost_orders bo where bo.id = $1 and $2 in (bo.buyer_id, bo.booster_id)))
      order by m.seq asc`,
    [boostId, userId, admin]
  );
}

export async function listBoostOrders(userId) {
  const rows = await query(
    `select id, service, matches, amount, status, created_at
       from public.boost_orders where buyer_id = $1 order by created_at desc`,
    [userId]
  );
  return rows.map((r) => ({ ...r, amount: money(r.amount) }));
}

// ───────────── Landing / social proof ─────────────
// Сэтгэгдэлтэй сүүлийн review-ууд (testimonials). Хоосон бол хэсгийг нууна.
export const getRecentReviews = unstable_cache(async function getRecentReviews(limit = 6) {
  return query(
    `select r.id, r.stars, r.comment, p.display_name as seller_name
       from public.reviews r
       left join public.public_profiles p on p.id = r.seller_id
      where r.comment is not null and char_length(btrim(r.comment)) > 1
      order by r.created_at desc limit $1`,
    [limit]
  );
}, ["recent-reviews"], { revalidate: 60 });

// Сүүлд зарагдсан зарууд (recent-sales ribbon). Нэргүй (зөвхөн гарчиг+үнэ).
export const getRecentSales = unstable_cache(async function getRecentSales(limit = 8) {
  const rows = await query(
    `select id, title, price from public.listings
      where status = 'sold' and deleted_at is null
      order by updated_at desc limit $1`,
    [limit]
  );
  return rows.map((r) => ({ id: r.id, title: r.title, price: Number(r.price) }));
}, ["recent-sales"], { revalidate: 60 });

// ───────────── Landing / market stats (live counters) ─────────────
// Бодит тоо: идэвхтэй зар, дууссан арилжаа, escrow-оор дамжсан нийт дүн.
export const getMarketStats = unstable_cache(async function getMarketStats() {
  const row = await queryOne(
    `select
       (select count(*) from public.listings
         where status = 'active' and deleted_at is null)::int as listings,
       (select count(*) from public.orders where status = 'completed')::int as trades,
       coalesce((select sum(amount) from public.orders
                  where status in ('paid','transferring','inspecting','completed')), 0)::bigint as protected`
  );
  return {
    listings: Number(row?.listings ?? 0),
    trades: Number(row?.trades ?? 0),
    protected: Number(row?.protected ?? 0),
  };
}, ["market-stats"], { revalidate: 60 });
