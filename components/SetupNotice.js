...
export default async function SetupNotice() {
  ...
  return (
    <div className="rounded-xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900">
      ...
      <p className="mb-3 text-amber-800/90">{t("setup.intro")}</p>
      <ol className="list-decimal space-y-1 pl-5 text-amber-800/80">
        {steps.map((s, i) => (
          <li key={i}>
            <code className="rounded bg-amber-100 px-1 text-amber-900">{s}</code>
          </li>
        ))}
      </ol>
    </div>
