...
export default function QPayPay({ orderId, kind }) {
  ...
  async function pay() {
    setBusy(true);
    setErr(null);
    const r = await call(createQpayInvoice, orderId);
    setBusy(false);
    if (r.error) {
  ...
  // route handler-ээр (server action-аар poll хийвэл client талын action дараалал
  // блоклогддог). Алдааг чимээгүй өнгөрөөж дараагийн оролдлогод үлдээнэ.
  useEffect(() => {
    if (!inv) return;
    let stopped = false;
    const tick = async () => {
      ...
      try {
        const res = await fetch("/api/payments/check", {
          ...
          body: JSON.stringify({ order: orderId, kind }),
        });
  ...
  async function recheck() {
    ...
    setErr(null);
    setInfo(null);
    const r = await call(checkPaymentNow, orderId, kind);
    setChecking(false);
  ...
  return (
    <div className="space-y-2">
      <button
        onClick={pay}
        disabled={busy}
        className="w-full rounded-lg bg-gradient-to-r from-violet to-azure px-4 py-3 text-sm font-semibold text-white hover:brightness-110 disabled:opacity-50"
      >
