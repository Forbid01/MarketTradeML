...
export default function MobileMenu({ authed, displayName, role, isVerified, unread = 0 }) {
  ...
  const link = "flex items-center gap-2 rounded-lg px-3 py-2.5 text-sm text-slate-700 hover:bg-slate-50";
  ...
  return (
    <div className="md:hidden">
      <button
        ...
        className="relative flex h-10 w-10 items-center justify-center rounded-lg text-slate-700 hover:bg-slate-50"
      >
      ...
      {open && (
        <>
          <div className="fixed inset-0 z-30 bg-slate-900/20" onClick={close} aria-hidden />
          <nav className="absolute left-0 right-0 top-14 z-40 mx-2 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
            ...
            {authed ? (
              <>
                ...
                {role === "admin" && (
                  <Link href="/admin" className={`${link} text-amber-600`} onClick={close}>{t("nav.admin")}</Link>
                )}
                <Link href="/account" className={link} onClick={close}>
                  {displayName ?? t("nav.profile")}
                  {isVerified && <BadgeCheck size={16} className="text-blue-600" />}
                </Link>
              </>
            ) : (
