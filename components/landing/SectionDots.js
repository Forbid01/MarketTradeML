      ...
      {ids.map((id, i) => {
        ...
        return (
          <button key={id} type="button" onClick={() => go(id)} aria-label={label} aria-current={on} className="group flex items-center justify-end gap-2">
            <span className={`text-[10px] font-semibold uppercase tracking-widest transition ${on ? "text-[#38BDF8]" : "text-slate-600 group-hover:text-slate-400"}`}>
              {String(i + 1).padStart(2, "0")} {label}
            </span>
            <span className={`h-2.5 w-2.5 rounded-full border transition-all ${on ? "scale-125 border-[#38BDF8] bg-[#38BDF8] motion-safe:animate-[dotPulse_2.4s_ease-in-out_infinite]" : "border-white/25 group-hover:border-[#38BDF8]/60"}`} />
          </button>
        );
      })}
    </nav>
