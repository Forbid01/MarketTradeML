  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-azure">{t("landing.faqEyebrow")}</p>
        <h2 className="mt-3 text-3xl font-extrabold uppercase tracking-tight text-slate-50 sm:text-4xl">{t("landing.faqTitle")}</h2>
      </div>
      <div className="mt-10 space-y-3">
        {list.map((it, i) => (
          <details key={i} className="group rounded-xl border border-white/10 bg-white/[0.03] p-4 transition hover:border-violet/30">
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3 text-sm font-medium text-slate-100 [&::-webkit-details-marker]:hidden">
              {it.q}
              <Plus size={18} className="shrink-0 text-azure transition-transform duration-300 group-open:rotate-45" />
            </summary>
