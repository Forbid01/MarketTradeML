...
export default async function AdminPage() {
  ...
  const [{ data: disputes }, { data: payouts }] = await Promise.all([
    supabase
      .from("disputes")
      .select("id, reason, status, created_at, order_id, orders(listings(title))")
      .eq("status", "open")
      .order("created_at", { ascending: true }),
    supabase
      .from("orders")
      .select("id, amount, fee, created_at, listings(title)")
      .eq("status", "completed")
      ...
      <section className="space-y-2">
        ...
        {!disputes?.length ? (
          ...
          disputes.map((d) => (
            ...
            >
              <p className="font-medium text-slate-900">{d.orders?.listings?.title ?? "Захиалга"}</p>
      ...
      <section className="space-y-2">
        ...
        {!payouts?.length ? (
          ...
          payouts.map((o) => (
            ...
            >
              <span className="text-slate-900">{o.listings?.title ?? "Захиалга"}</span>
